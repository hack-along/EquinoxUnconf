<template>
  <div>
    <div class="graph" ref="timelinechartdiv" id="timelinechartdiv"></div>
    <button v-on:click="updateSeries">update</button>
  </div>
</template>

<script>
import events from "@/data/status.json";
import processed from "@/data/processedevents.json";
...
export default {
  ...
  methods: {
      ...
      for (let i = 0; i < events.length; i++) {
        ...
        if (event.action == "moved") {
          output.push(
            JSON.parse(
              '{ "from": "' +
                escape(event.fromroomname) +
                '", "to": "' +
                escape(event.toroomname) +
                '", "value": "1"}'
            )
          );
          console.log("added");
    ...
    updateSeries: function () {
      var output = [];
      for (let i = 500; i < events.length; i++) {
        var event = events[i];

        if (event.action == "joined" || event.action == "left") {
          if (!output[event.roomname]) {
            // check room existence
            output[event.roomname] = [];
            output[event.roomname].name = event.roomname,
            output[event.roomname].children = [];
            output[event.roomname].link = [];
          }
          ...
          if (
            ...
            output[event.roomname].children.push(obj); // avoid duplicates
        }

        if (event.action == "moved") {
          if (!output[event.toroomname]) {
            // check room existence
            output[event.toroomname] = [];
            output[event.toroomname].name = event.toroomname;
            output[event.toroomname].children = [];
            output[event.toroomname].link = []
          }
          else 
            output[event.toroomname].link.push(event.fromroomname)
          var obj = JSON.parse(
            '{ "name": "' +
              escape(event.name) +
              '", "image": "' +
          ...
          if (
            ...
            // avoid duplicates
            output[event.toroomname].children.push(obj);
        }
      }
  ...
  mounted() {
    am4core.ready(() => {
      // Themes begin
    this.chart = am4core.create("timelinechartdiv", am4plugins_timeline.CurveChart);
    ...
    categoryAxis.renderer.radius = 90;

    categoryAxis.renderer.tooltipLocation = 0;
    categoryAxis.renderer.line.strokeDasharray = "1,4";
    categoryAxis.renderer.line.strokeOpacity = 0.5;
    categoryAxis.tooltip.background.fillOpacity = 0.2;
    categoryAxis.tooltip.background.cornerRadius = 5;
    categoryAxis.tooltip.label.fill = new am4core.InterfaceColorSet().getFor("alternativeBackground");
    categoryAxis.tooltip.label.paddingTop = 7;
    ...
    categoryAxis.renderer.tooltipLocation2 = 0;
